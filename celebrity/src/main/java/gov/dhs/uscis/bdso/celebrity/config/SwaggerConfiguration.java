...
public class SwaggerConfiguration {
    ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("Celebrity API").description("API to retrieve celebrity information")
    ...
    public Docket customImplementation(ServletContext servletContext,
            @Value("${openapi.celebrity.base-path:/api/v1}") String basePath) {
        return new Docket(DocumentationType.SWAGGER_2)
                .select()
                .apis(RequestHandlerSelectors.basePackage("gov.dhs.uscis.bdso.celebrity.api")).build()
